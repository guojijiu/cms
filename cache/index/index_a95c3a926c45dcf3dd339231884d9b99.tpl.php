		...
		<div class="box">
			...
			<div class="swiper-container yzm-banner">
				<div class="swiper-wrapper">
					<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'banner')) {$data = $tag->banner(array('field'=>'title,image,url,typeid','limit'=>'5',));}?>
			...
			<div class="yzm-content-box yzm-top-right">
				...
				<ul class="yzm-ranking">
					...
					<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'all')) {$data = $tag->all(array('field'=>'title,url,inputtime','limit'=>'10',));}?>
					<?php if(is_array($data)) foreach($data as $k=>$v) { ?>
					<?php $k=$k+1;?>
					   <li><em><?php echo $k;?></em><span class="date"><?php echo date('m-d',$v['inputtime']);?></span><a href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" target="_blank"><?php echo $v['title'];?></a></li>	
					<?php } ?>	
				</ul>
			</div>
			...
			<div class="yzm-content-box yzm-text-thumbs">
				...
				<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'lists')) {$data = $tag->lists(array('field'=>'title,description,url,color,thumb','modelid'=>'1','flag'=>'4','limit'=>'4',));}?>
				<?php if(is_array($data)) foreach($data as $v) { ?>
			...
			<div class="yzm-content-box yzm-tag">
			    ...
			    <ul>
				<?php $tag = yzm_base::load_sys_class('yzm_tag');if(method_exists($tag, 'tag')) {$data = $tag->tag(array('field'=>'id,tag,total','limit'=>'27',));}?>
				<?php if(is_array($data)) foreach($data as $v) { ?>
			...
			<div class="yzm-content-box yzm-link">
				...
				<ul>联系地址：广州市越秀区寺右南一街一巷9号广日大厦9楼902A室</ul>
				<ul>联系电话：020-3762 9980</ul>
				<ul>邮政编码：510 600</ul>
			</div>			  
		</div>
