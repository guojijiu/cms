...
<div class="yzm-header-box">
	<div class="yzm-header-top">
		...
		<a href="<?php echo url_referer(1);?>"  target="_blank" class="login">登录</a>
		<?php } else { ?>
		你好：<?php echo safe_replace(get_cookie('_username'));?>，<a href="<?php echo U('member/index/init');?>">会员中心</a> <a href="<?php echo url_referer(0);?>">退出</a>
		<?php } ?>
		</div>
		<div class="topleft">
			广州市电梯行业协会：<a href="http://1.12.181.127/" target="_blank">广州市电梯行业协会</a>
		</div>
	</div>
</div>
<div class="yzm-container-box">
	...
	<div class="yzm-header">
		<div class="yzm-logo">
		 <!-- 调用站点Logo标签，请用：<?php echo $site['site_logo'];?> -->
		 <a href="<?php echo SITE_URL;?>"><img src="<?php echo $site['site_logo'];?>" title="<?php echo $site['site_name'];?>" alt="<?php echo $site['site_name'];?>"></a>
		</div>
	...
	<div class="yzm-menu">
	  <ul class="yzm-nav">
		...
		<?php if(is_array($data)) foreach($data as $v) { ?>
		    <li>
				<a <?php if(isset($catid) && in_array($catid, explode(',', $v['arrchildid']))) { ?> class="current" <?php } ?>  target="<?php echo $v['target'];?>"><?php echo $v['catname'];?></a>
				<!-- 这里是二级栏目的循环，不需要的可以删除，代码开始 -->
				<?php if(is_childid($v)) { ?> 
				<?php $r = get_childcat($v['catid']);?>
				<ul class="sub_nav">
					<?php if(is_array($r)) foreach($r as $v) { ?>
					<li><a href="<?php echo $v['pclink'];?>" target="<?php echo $v['target'];?>"><?php echo $v['catname'];?></a></li>
					<?php } ?>	
				</ul>
				<?php } ?>
